...
table(orig_email_rec$Unsubscribed)
table(orig_email_rec$Bounce.Category)
...
Stats on splits:
```{r}
table(orig_email_rec$Split.Name)
```
...
#merged[merged$donated_TY_pre_treatment==1,]
table(merged$Split.Name,merged$donated_TY_pre_treatment)

table(merged$donated_after_reminder2,merged$Split.Name)
```
...
reg_remember_treatment1<-lm(opened_reminder1~merged$Split.Name,data=merged)
#print(summary(reg_remember_treatment))

reg_remember_treatment2<-lm(opened_reminder2~merged$Split.Name,data=merged)
#print(summary(reg_remember_treatment))

reg_remember_treatment_any<-lm(opened_atleast_one_reminder~merged$Split.Name,data=merged)
#print(summary(reg_remember_treatment))

stargazer(reg_remember_treatment1, reg_remember_treatment2, reg_remember_treatment_any, type="text",              dep.var.labels=c("Opened Reminder1","Opened Reminder2 indicator","Opened any reminder indicator"),              covariate.labels=c("Split"),out="bad_control.txt") 
...
reg_response_m1<-glm(donated_any_time~treat+opened_orig_email+opened_atleast_one_reminder+PGSS_age+last_gift_amount+treat_Ben_inter+SYBUNT_Indicator+LYBUNT_indicator+donated_TY_pre_treatment+gift_count, data=compliant_orig,family="binomial")
print(printrobustSE(reg_response_m1))

#Method2: all emails
#THis is intent to treat
reg_response_m2<-glm(donated_any_time~treat+opened_atleast_one_reminder+PGSS_age+last_gift_amount+treat_Ben_inter+SYBUNT_Indicator+LYBUNT_indicator+donated_TY_pre_treatment+gift_count, data=merged,family="binomial")
print(printrobustSE(reg_response_m2))
...
#Method1: exclude everyone who did not open original email
reg_amt_m1<-lm(total_donation_amount~treat+opened_atleast_one_reminder+PGSS_age+last_gift_amount+treat_Ben_inter+SYBUNT_Indicator+LYBUNT_indicator+donated_TY_pre_treatment+gift_count, data=compliant_orig)
#print(printrobustSE(reg_amt_m1))

#Methods: all
#THis is intent to treat
reg_amt_m2<-lm(total_donation_amount~treat+opened_atleast_one_reminder+PGSS_age+last_gift_amount+treat_Ben_inter+SYBUNT_Indicator+LYBUNT_indicator+donated_TY_pre_treatment+gift_count, data=merged)
print(printrobustSE(reg_amt_m2))
print(summary(reg_amt_m2))
...
just_msg_effect_immediate<-lm(donated_after_orig_email~treat, data=merged)
print("Original treatment effect on immediate response:")
print(printrobustSE(just_msg_effect_immediate))
...
stargazer(just_msg_effect_immediate, reg_response_m2, reg_amt_m2, dep.var.labels  =c("Immediate donation $","Donated anytime","Total donation $"),out="reg.txt") 
...
```{r}
reg_delay<-lm(days_till_donation~treat+treat_reminder+opened_reminder1+opened_reminder2+PGSS.Year+Months_since_last_donation+last_gift_amount, data=merged)
...
```{r}
table(merged$opened_orig_email,merged$Split.Name)

table(merged$opened_orig_email,merged$donated_after_orig_email)
...
t-test for original email effect
```{r}
t.test(merged$donated_after_orig_email,by=merged$Split.Name)

#install.packages("tidyr")
#library(tidyr)
#stargazer(tidy(result),out="ttest.txt")

nrow(compliant_orig)

t.test(compliant_orig$donated_after_orig_email,by=compliant_orig$Split.Name)

```
```{r}

prop.test(x = c(17, 7), n = c(1053, 1054),
              alternative = "greater")
```